...
export function ContactCta() {
  return (
    <AnimatedSection id={siteContent.contact.id} className="relative py-16 sm:py-32">
      <div className="absolute inset-0 -z-10">
        <div className="absolute inset-0 hidden md:block">
          <Image src={siteContent.contact.ctaImages.desktop} alt="CTA financiero" fill className="object-cover object-center opacity-[0.16] saturate-[0.98]" sizes="100vw" />
        </div>
        <div className="absolute inset-0 md:hidden">
          <Image src={siteContent.contact.ctaImages.mobile} alt="CTA financiero móvil" fill className="object-cover object-center opacity-[0.18] saturate-[0.98]" sizes="100vw" />
        </div>
        <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(251,251,246,.72),rgba(246,250,242,.92)_24%,rgba(236,244,229,.98)_100%)]" />
      </div>

      <Container className="grid items-start gap-8 sm:gap-12 lg:grid-cols-[.88fr_1.12fr]">
        <div>
...
function InfoCard({ icon, label, value, href, secondary }: { icon: ReactNode; label: string; value: string; href?: string; secondary?: string }) {
  const content = (
    <>
      <div className="rounded-[1.1rem] border border-[#0f4e2f]/8 bg-white/92 p-3.5 shadow-[0_12px_30px_rgba(18,52,34,0.06)]">{icon}</div>
      <div>
  ...
  return (
    <a href={href} target={href.startsWith("http") ? "_blank" : undefined} rel={href.startsWith("http") ? "noreferrer" : undefined} className="crece-panel-soft flex items-center gap-4 rounded-[1.55rem] px-4 py-4 sm:rounded-[1.75rem] sm:px-5 sm:py-5 text-left transition-colors duration-300 hover:bg-white">
      {content}
