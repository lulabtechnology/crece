...
import { Building2, HandCoins, LineChart } from "lucide-react";
...
const icons = [Building2, LineChart, HandCoins];

export function InstitutionalOverview() {
  return (
    <AnimatedSection id={siteContent.institutional.id} className="crece-section-band relative py-14 sm:py-24">
      <Container className="grid items-start gap-8 sm:gap-12 lg:grid-cols-[1.02fr_.98fr]">
        <div>
          <SectionHeading
            eyebrow="Consultoría"
          ...
          <RevealList className="mt-6 grid gap-4 sm:mt-8 md:grid-cols-3">
              ...
              return (
                <RevealItem key={item.title}>
                  <article className="crece-panel crece-premium-stroke h-full rounded-[1.6rem] p-5 sm:rounded-[1.8rem] sm:p-6">
                    ...
                    <h3 className="crece-display mt-5 text-[1.32rem] font-semibold text-[#173225]">{item.title}</h3>
        ...
        <div className="relative">
          ...
          <div className="crece-panel relative mt-4 max-w-md rounded-[1.35rem] p-4 sm:absolute sm:-bottom-6 sm:left-6 sm:mt-0 sm:rounded-[1.5rem] sm:p-5">
